...
**Note:** In case you encounter any problems with our automated local tunneling solution, feel free to explore an alternative option using ngrok, which can be obtained from the provided link: https://ngrok.com/download.  
Once you've installed ngrok, utilize the following command to establish a tunnel:   
 ```bash  
 ngrok http 8080   
 ``` 
You will be presented with a tunnel link to your localhost. Replace the `URL` parameter in the `.env` file with this link, allowing the  server to restart automatically.  
